...
struct ButtonStatusColor: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        case .idle:
            return content.allowButton(
                foregroundColor: colorStore.buttonIdle.foregroundColor,
                backgroundColor: colorStore.buttonIdle.backgroundColor
            )
            
        case .allowed:
            return content.allowButton(
                foregroundColor: colorStore.buttonAllowed.foregroundColor,
                backgroundColor: colorStore.buttonAllowed.backgroundColor
            )
            
        case .denied:
            return content.allowButton(
                foregroundColor: colorStore.buttonDenied.foregroundColor,
...
struct AllowButton: ViewModifier {
    ...
    var buttonSizeConstant: CGFloat {
        screenSize.width < 400 ?  70 - (1000 - screenSize.width) / 30 : 70
    }
