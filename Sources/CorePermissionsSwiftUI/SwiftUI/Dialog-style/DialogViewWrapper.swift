...
@usableFromInline struct DialogViewWrapper<Body: View>: View, CustomizableView {
    ...
    init(for bodyView: ViewType, showing: Binding<Bool>, store: PermissionStore) {
        ...
        self.store = store
        self.schemaStore = PermissionSchemaStore(
            store: store,
    ...
    @usableFromInline var body: some View {
        ZStack {
            let insertTransition = AnyTransition.opacity.combined(with: .scale(scale: 1.1)).animation(.default.speed(1.6))
            let removalTransiton = AnyTransition.opacity.combined(with: .scale(scale: 0.9)).animation(.default.speed(1.8))
            ...
            if shouldShowPermission {
                Group {
                    ...
                    Blur(style: .systemUltraThinMaterialDark)
                        .transition(.opacity.animation(.default.speed(1.6)))
#else
