...
final class MockHealthManager: HealthManager {
    ...
    var requestSuccessOverride = true
    static var healthDataAvailableOverride = true
    ...
    func authorizationStatus(for type: HKObjectType) -> HKAuthorizationStatus {
        ...
        case .authorized: .sharingAuthorized
        case .notDetermined: .notDetermined
        case .denied: .sharingDenied
        case .mixedAuthorized: getCorrectPermission()
        case .mixedDenied: getCorrectPermission()
        }
    }
    ...
    func requestAuthorization(toShare typesToShare: Set<HKSampleType>?, read typesToRead: Set<HKObjectType>?, completion: @escaping (Bool, Error?) -> Void) {
        ...
        if requestSuccessOverride {
            ...
            guard typesToShare != nil || typesToRead != nil else {
                return
            }
